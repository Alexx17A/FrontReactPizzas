  ...
  if (loading) {
    return (
      <div className="d-flex">
        <Sidebar />
        <div className="main-content p-4">Loading categories...</div>
      </div>
    );
  }
              ...
              {categories.length > 0 ? (
                categories.map(category => (
                  <tr key={category.categoryId}>
                    ...
                    <td>
                      <button className="btn btn-warning btn-sm me-2" onClick={() => handleEdit(category)}>Editar</button>
                      <button className="btn btn-danger btn-sm" onClick={() => handleDelete(category.categoryId)}>Eliminar</button>
                    </td>
                  </tr>
                ))
              ) : (
                <tr><td colSpan="4" className="text-center text-muted">No hay categorías registradas.</td></tr>
              )}
            </tbody>
          </table>
        </div>
